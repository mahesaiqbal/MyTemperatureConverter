package com.mahesaiqbal.mytemperatureconverter
...
import com.mahesaiqbal.mytemperatureconverter.ui.theme.MyTemperatureConverterTheme
...
// Stateful version
// Below is an example of implementing Unidirectional Data Flow (UDF) in Jetpack Compose
@Composable
fun StatefulTemperatureInput(
    ...
    Column(modifier.padding(16.dp)) {
        ...
        OutlinedTextField(
            value = input, // Showing state
            label = { Text(stringResource(R.string.enter_celsius)) },
            keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number),
            onValueChange = { newInput -> // Event to update the state
                input = newInput // Update state
...
// Stateful with Stateless composable (State Hoisting)
@Composable
...
// General stateless composable for temperature input
@Composable
...
// Two way stateful composable by calling stateless general stateless composable
@Composable
private fun TwoWayConverterApp(
    modifier: Modifier = Modifier
) {
    // Use rememberSaveable (For example), to maintain the state when the screen orientation changes
